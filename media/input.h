...
///     - false : Gamepad is inactive.
attr_header m_bool32 input_gamepad_rumble_clear( m_uint32 index ) {
...
attr_header
attr_always_inline m_bool32 keyboard_state_get_key(
    const KeyboardState* state, KeyboardCode code
) {
    m_uint32 index = (m_uint32)code;
...
attr_always_inline void keyboard_state_set_key( 
    KeyboardState* state, KeyboardCode code, m_bool32 value
) {
    m_uint32 index = (m_uint32)code;
    if( !index || index >= KB_COUNT ) {
        return;

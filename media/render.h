...
#define MEDIA_OPENGL_DEFAULT_MAJOR_VERSION (3)
/// @brief Default minor version of OpenGL.
#define MEDIA_OPENGL_DEFAULT_MINOR_VERSION (2)
...
typedef struct { u8 raw[sizeof(int) * 16]; } MediaOpenGLAttributes;
...
attr_media_api b32 media_render_gl_attr_set(
    MediaOpenGLAttributes* attr, MediaOpenGLAttribute name, int value );
...
attr_media_api int media_render_gl_attr_get(
    MediaOpenGLAttributes* attr, MediaOpenGLAttribute name );
...
attr_media_api OpenGLRenderContext* media_render_gl_context_create(
    MediaSurface* surface, MediaOpenGLAttributes* opt_attributes );
